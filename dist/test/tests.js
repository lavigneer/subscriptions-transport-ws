...
describe('Server', function () {
    ...
    it('should call unsubscribe when client closes the connection', function (done) {
        ...
        setTimeout(function () {
            chai_1.assert(spy.calledTwice);
            done();
        }, 1000);
    });
    it('should remove server side operation after GQL_COMPLETE is sent', function (done) {
        var client = new client_1.SubscriptionClient("ws://localhost:" + EVENTS_TEST_PORT + "/");
        client.request({
            query: "query testString {\n        testString\n      }",
        ...
        setTimeout(function () {
            chai_1.assert(eventsOptions.onOperationComplete.calledOnceWith(sinon.match.any, '1'));
            done();
        }, 500);
    });
